...
on run
	...
	on error
		...
		on error
			display dialog "Impossible de trouver photo_sorter.py à côté du lanceur." with title "Photos Trieur" buttons {"OK"} default button "OK"
			return
		end try
	end try
	
	set sourceDir to my chooseFolderWithPrompt("Étape 1/4 : choisir le dossier source")
	if sourceDir is "__CANCEL__" then return
	
	set outputFolderPath to my chooseFolderWithPrompt("Étape 2/4 : choisir le dossier cible final (existant ou nouveau)")
	...
	set mediaScope to my chooseButton("Étape 3/4 : inclure aussi les vidéos ?", {"Annuler", "Photos seules", "Photos et vidéos"}, "Photos seules", "Annuler")
	if mediaScope is "__CANCEL__" then return
	
	set modeChoice to my chooseButton("Étape 4/4 : choisir le mode", {"Annuler", "Prévisualiser", "Exécuter"}, "Prévisualiser", "Annuler")
	...
	if mediaScope is "Photos et vidéos" then
		set cmd to cmd & " --include-videos"
	end if
	if modeChoice is "Exécuter" then
	...
	set notifyFailure to "display notification \"Traitement échoué (" & modeChoice & "). Voir le journal.\" with title \"Photos Trieur\""
